...
import { one, mim } from "../../../helpers/tokens";
...
function Zapin({ open, handleClose, bond }: IZapinProps) {
    ...
    let defaultToken = tokens.find(token => token.name === one.name);

    if (bond.name === wone.name) {
    ...
    const setMax = () => {
        ...
        let amount: any = Math.min(maxBondPriceToken, token.isONE ? token.balance * 0.99 : token.balance);
